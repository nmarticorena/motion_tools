"""Based on https://github.com/mujocolab/g1_spinkick_example
but for the GMR motions"""
...
def main(
    ...
    with open(pkl_file, "rb") as f:
        ...
        fps = data["fps"]
        pos = data["root_pos"]  # (N, 3)
        rot = data["root_rot"]  # (N, 4) - quat [x, y, z, w]
        rot_rot_vec = Rotation.from_quat(
            rot, scalar_first=False
        ).as_rotvec()  # (N, 3) - axis-angle
        joints = data["dof_pos"]  # (N, 29)

        steps = data["root_pos"].shape[0]
        frames = np.hstack([pos, rot_rot_vec, joints])

    original_duration = steps / fps

    print(f"Loaded motion with steps: {steps}")
    print(f"FPS: {fps}")
    ...
    if add_start_transition:
        ...
        start_rot = start_rot_obj.as_quat(scalar_first=True)
